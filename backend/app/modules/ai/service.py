...
class AIService:
    ...
    def _get_meeting_context(self, meeting_id: uuid.UUID, allowed_user_ids: list[uuid.UUID] | None) -> str:
        ...
        # Fetch all notes for the meeting within scope
        notes = self._notes.list_scoped(allowed_user_ids=allowed_user_ids, meeting_id=meeting_id)
    ...
    def generate_meeting_summary(self, meeting_id: uuid.UUID, allowed_user_ids: list[uuid.UUID] | None) -> str:
        notes_content = self._get_meeting_context(meeting_id, allowed_user_ids)
        if not notes_content:
            return "No notes available to summarize."
        return self._llm.generate_meeting_summary(notes_content)

    def extract_action_items(self, meeting_id: uuid.UUID, allowed_user_ids: list[uuid.UUID] | None) -> list[str]:
        notes_content = self._get_meeting_context(meeting_id, allowed_user_ids)
        if not notes_content:
            return ["No notes available to extract action items."]
        return self._llm.extract_action_items(notes_content)

    def draft_follow_up_email(self, meeting_id: uuid.UUID, allowed_user_ids: list[uuid.UUID] | None) -> str:
        meeting = self._meetings.get_by_id(meeting_id)
        if meeting is None or not is_in_rbac_scope(meeting.created_by_user_id, allowed_user_ids):
            raise MeetingNotFoundError("Meeting not found")
    ...
    def generate_customer_insights(self, customer_id: uuid.UUID, allowed_user_ids: list[uuid.UUID] | None) -> str:
        ...
        meetings = self._meetings.list_scoped(allowed_user_ids=allowed_user_ids, customer_id=customer_id)
        ...
        for m in meetings:
            ...
            notes = self._notes.list_scoped(allowed_user_ids=allowed_user_ids, meeting_id=m.id)
            if notes:
                for n in notes:
